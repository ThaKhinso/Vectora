		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession) {
				...
				if (Log::GetCoreLogger()) { // Edge case: BeginSession() might be before Log::Init()
					VE_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			if (m_OutputStream.is_open()) {
				...
				if (Log::GetCoreLogger()) { // Edge case: BeginSession() might be before Log::Init()
					VE_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
