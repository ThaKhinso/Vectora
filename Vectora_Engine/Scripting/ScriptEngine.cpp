...
namespace Vectora {
	...
	namespace Utils {
		ScriptFieldType MonoTypeToScriptFieldType(MonoType* monoType)
		{
			...
			{
				VE_CORE_ERROR("Unknown type: {}", typeName);
		...
		static MonoAssembly* LoadMonoAssembly(const std::filesystem::path& assemblyPath)
		{
			...
			std::string tempPath = assemblyPath.string();
			MonoAssembly* assembly = mono_assembly_load_from_full(image, tempPath.c_str(), &status, 0);
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		int32_t numTypes = mono_table_info_get_rows(typeDefinitionsTable);
		MonoClass* entityClass = mono_class_from_name(s_Data->CoreAssemblyImage, "Vectora", "Entity");
		...
		for (int32_t i = 0; i < numTypes; i++)
		{
			...
			VE_CORE_WARN("{} has {} fields:", className, fieldCount);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					VE_CORE_WARN("  {} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
