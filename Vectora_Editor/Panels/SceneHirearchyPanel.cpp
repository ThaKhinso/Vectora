...
namespace Vectora {
	...
	void SceneHirearchyPanel::DrawEntityNode(Entity entity)
	{
		...
		if (ImGui::BeginPopupContextItem(0, ImGuiPopupFlags_MouseButtonRight | ImGuiPopupFlags_NoOpenOverItems))
		{
	...
	void SceneHirearchyPanel::DrawComponents(Entity entity)
	{
		...
		if (ImGui::BeginDragDropTarget())
		{
			if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
			{
				...
				component.texPath = texturePath;
				Ref<Texture2D> texture = Texture2D::Create(texturePath.string());
				if (texture->IsLoaded())
					component.Texture = texture;
				else
					VE_WARN("Could not load texture {0}", texturePath.filename().string());
			}
	...
	template<typename T>
	void SceneHirearchyPanel::DisplayAddComponentEntry(const std::string& entryName) {
